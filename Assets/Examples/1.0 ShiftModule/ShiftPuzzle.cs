...
public class ShiftPuzzle : MonoBehaviour
{
	...
	bool checkWin(){
		...
		case 32:
			...
			break;
		case 33:
			if (field [2] == 1 && field [5] == 2 && field [8] == 3&&field[4]==4&&field[7]==3)
				return true;
			break;
		default:
			return false;
		...
		for (int i = 0; i <= 8; i++) {
			for (int j = 0; j <= 8; j++) {
				if (start [j] == field [i]) {
					if (start [j] == 0) {
						if (start [i] != 0) {
							dif = buttons [start [i] - 1].gameObject.transform.position - emptyField.transform.position;
							buttons [start [i] - 1].gameObject.transform.Translate (-dif, Space.World);
							emptyField.transform.Translate (dif, Space.World);
						}
					} else if (start [i] != 0) {
						dif = buttons [start [i] - 1].gameObject.transform.position - buttons [start [j] - 1].gameObject.transform.position;
						buttons [start [i] - 1].gameObject.transform.Translate (-dif, Space.World);
						buttons [start [j] - 1].gameObject.transform.Translate (dif, Space.World);
					} else {
						dif = emptyField.transform.position - buttons [start [j] - 1].gameObject.transform.position;
						emptyField.transform.Translate (-dif, Space.World);
						buttons [start [j] - 1].gameObject.transform.Translate (dif, Space.World);
					}
			...
			for (int i = 0; i <= 8; i++) {
				if (field [i] == button + 1) {
					...
					buttons [button].gameObject.transform.Translate (0, 0, emptyZ-temp, Space.Self);*/
						...
						dif = buttons [button].gameObject.transform.position - emptyField.transform.position;
						buttons [button].gameObject.transform.Translate (-dif, Space.World);
						emptyField.transform.Translate (dif, Space.World);
					}
					break;
				}
